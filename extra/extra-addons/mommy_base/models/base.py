...
class BaseModel(models.AbstractModel):
    ...
    def show_notification(self, title, type="info", sticky=False):
        """
    ...
    def get_binary_share_url(self, field_name):
        """
        Get binary share url for the attachment field
        """
        attachment = getattr(self, field_name)
        if not attachment:
            return False
        ...
        if not ir_attachment:
            return False
        share = self.env['attachment.share'].sudo().search([('attachment_id', '=', ir_attachment.id)], limit=1)
        if not share:
            share = self.env['attachment.share'].sudo().create({
                'attachment_id': ir_attachment.id,
            })
        return share.download_url
    
    def open_url(self, url, target='new'):
        """
        Open URL action
        """
        return {
    ...
    def get_qrcode_base64(self, content, img_type='PNG'):
        """
        Get QRCode binary for the content
        
        :return bytes: QRCode binary data
        """
        img = qrcode.make(content).get_image()
        buffer = BytesIO()
