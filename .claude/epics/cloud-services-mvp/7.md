...
- [ ] PaaS Operator HTTP client 實現（Python requests wrapper）
- [ ] Cloud Service JSON API endpoints 實現：
  - [ ] GET `/woow/api/cloud/templates` - 取得應用模板列表
  - [ ] GET `/woow/api/cloud/templates/{id}` - 取得單一模板
  - [ ] GET `/woow/api/workspaces/{id}/services` - 取得服務列表
  - [ ] POST `/woow/api/workspaces/{id}/services` - 建立服務
  - [ ] GET `/woow/api/workspaces/{id}/services/{service_id}` - 取得服務詳情
  - [ ] PATCH `/woow/api/workspaces/{id}/services/{service_id}` - 更新服務
  - [ ] DELETE `/woow/api/workspaces/{id}/services/{service_id}` - 刪除服務
  - [ ] POST `/woow/api/workspaces/{id}/services/{service_id}/rollback` - 回滾
  - [ ] GET `/woow/api/workspaces/{id}/services/{service_id}/revisions` - 版本歷史
- [ ] 部署流程整合（Odoo → Operator → 狀態輪詢）
- [ ] 錯誤處理與狀態同步
...
- [ ] Operator client 可成功與 PaaS Operator 通訊
- [ ] 所有 API endpoints 可正常回應
- [ ] 部署流程可正常執行（從 pending 到 running）
- [ ] 錯誤處理正確（state=error + error_message）
- [ ] 設定可透過 Odoo Settings 頁面配置
